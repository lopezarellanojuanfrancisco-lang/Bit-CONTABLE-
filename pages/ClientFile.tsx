...
import { Nota, EtapaContable, SatCertificates, PagoCliente, Ticket, TicketStatus } from '../types';
...
export const ClientFile: React.FC = () => {
  ...
  // UI STATE - DEFAULT TAB IS NOW CHAT
  ...
  const handleSendReportToChat = (fileUrl: string, message: string) => {
     // Switch to Chat Tab
     setActiveTab('CHAT');
     // Add Message with "Attachment"
     const attachmentMsg = `${message}\n\n📄 ADJUNTO: ${fileUrl}`;
     handleSendMessage(attachmentMsg);
  };
  ...
  const handleUpdateTicketStatus = (ticketId: string, status: TicketStatus) => {
    setClientTickets(prev => prev.map(t => t.id === ticketId ? { ...t, status } : t));
    alert(status === 'INVOICED' ? 'Ticket marcado como FACTURADO' : 'Ticket marcado como RECHAZADO');
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4">
            ...
            <div className="hidden md:flex items-center gap-3">
               ...
               {/* Chat Button (Redundant but kept for quick access) */}
               <button 
          ...
          {/* LOGIC ORDER: Chat -> Facturas -> Contabilidad -> Llaves -> Tickets -> Resto */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className={`
           ...
           ${activeTab === 'CHAT' ? 'bg-[#efeae2] dark:bg-slate-900 p-0' : activeTab === 'TICKETS' ? 'p-0 sm:p-2' : 'p-6 lg:p-8'}
        `}>
           
           {/* -- TAB 1: CHAT (Full Height) -- */}
           {activeTab === 'CHAT' && (
           ...
           {activeTab === 'CONTABILIDAD' && (
             <AccountingModule 
                onSendReport={handleSendReportToChat}
             />
           )}

           {/* -- TAB 4: CERTIFICADOS (LLAVES) -- */}
